...
SQL_ASSET_SAVE = 'insert into asset(sn,ip,os,hostname,cpu,ram,disk,machine_room_id,vendor,model,time_on_shelves,over_guaranteed_date,buiness,admin,status) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
...
def get_asset_by_id(_id):
    rt_cnt,rt_list = dbutils.execute_sql(SQL_ASSET_BY_ID,(_id,),True)
    assets = []
    for rt in rt_list:
        asset = dict(zip(SQL_ASSET_LIST_COLUMNS,rt))
        for key in 'time_on_shelves,over_guaranteed_date'.split(','):
...
def validate_asset_save(sn,ip,os,hostname,cpu,ram,disk,machine_room_id,vendor,model,time_on_shelves,over_guaranteed_date,buiness,admin,status):
    return True, ''

def asset_save(sn,ip,os,hostname,cpu,ram,disk,machine_room_id,vendor,model,time_on_shelves,over_guaranteed_date,buiness,admin,status):
    return dbutils.execute_sql(SQL_ASSET_SAVE,(sn,ip,os,hostname,cpu,ram,disk,machine_room_id,vendor,model,time_on_shelves,over_guaranteed_date,buiness,admin,status),False)

def asset_delete(aid):
    rt_cnt, rt_list = dbutils.execute_sql(SQL_ASSET_DELETE,(aid,),False)
    return rt_cnt != 0
