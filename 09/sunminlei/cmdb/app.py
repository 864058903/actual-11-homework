...
def user_view():
    ...
    print user.get('sex', 0)
    return render_template('user_view.html', id=user.get('id', 0), \
                           username=user.get('name', ''), age=user.get('age', 0), telephone=user.get('telephone', ''), \
                           sex=user.get('sex', 0), department=user.get('department', 0), \
                           birthday=user.get('birthday', ''), hobby=user.get('hobby'), email=user.get('email', ''), \
...
@app.route('/room/save/json/', methods=["post"])
def room_save_json():
    if session.get('user') is None: return redirect('/')
    roomname = request.form.get('roomname', '')
    addr = request.form.get('addr', '')
    ...
    ok, error = models.validate_room_save(roomname, addr, ip_ranges)
    if ok:
        models.room_save(roomname, addr, ip_ranges)
        return json.dumps({'code': 200})
    else:
        return json.dumps({'code': 400, 'error': error})
...
def asset_list():
    ...
    assests = models.get_assets()
    return json.dumps({'data': assests})
...
def asset_delete():
    ...
    rt_cnt = models.asset_delete(request.args.get('id', 0))
    if rt_cnt:
        return json.dumps({'code': 200})
    else:
