...
def user_save(username, password, age):
    rt_cnt, _ = dbutils.execute_sql(SQL_USER_SAVE, (username.strip(), age.strip(),  crypt.md5_str(password.strip()) ), False)
    return rt_cnt != 0
...
def monitor_host_list(ip):
    ...
    print 'categoy_list, cpu_list, disk_list, mem_list',categoy_list, cpu_list, disk_list, mem_list
    for line in rt_list:
