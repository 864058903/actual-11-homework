...
用户修改密码
'''
@app.route('/user/chpass/')
def user_chpass():
    uid = request.args.get('id', '')
    return render_template('user_chpass.html',id=uid)
    # return render_template('user_chpass.html')
@app.route('/user/chpass/save/',methods=['POST'])
def user_chpass_save():
    uid = request.form.get('id', '')
    oldpassword = request.form.get('password','')
    newpassword1 = request.form.get('newpassword1','')
    newpassword2 = request.form.get('newpassword2','')
    username = session['user'] # 防止修改其他人密码
    ok,error = models.validate_user_change_passwd(uid,oldpassword,newpassword1,newpassword2,username)
    if ok:
        models.user_change_passwd_save(uid,newpassword1)
        return redirect('/users/')
    return render_template('user_chpass.html',error=error,id=uid)
