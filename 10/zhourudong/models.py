...
用户修改密码前 检查合法性
'''
def validate_user_change_passwd(uid,oldpassword,newpassword1,newpassword2,username):
    # 检查是否是 修改自己密码防止修改他人密码
    tmp_user=get_user_by_id(uid)
    if tmp_user.get('name',0) !=username.get('username'):
        return False,u"不能修改非己密码"

    SQL_VIIDATE_USER_CHANGE_PASSWD = 'SELECT `password` FROM `zrd`.`user` WHERE  `id`=%s AND `password`=md5(%s)'
    sql = SQL_VIIDATE_USER_CHANGE_PASSWD
    args = (uid,oldpassword)
    rt_cnt,rt_list = execu_sql(sql,args,True)
    if not rt_list:
        return False,u'旧密码输入错误'

    if oldpassword == newpassword1:
        return False,u'新旧密码不能一致'
    if newpassword1.strip() == '':
...
保存用户密码修改
'''
def user_change_passwd_save(uid,newpassword1):
    SQL_USER_CHANGE_PASSWD_SAVE = 'UPDATE user SET `password`=MD5(%s) WHERE  `id`=%s'
    sql = SQL_USER_CHANGE_PASSWD_SAVE
    args = (uid,newpassword1)
    rt_cnt,rt_list = execu_sql(sql,args,False)
    return True
...
if __name__ == "__main__":
    ...
    print validate_user_change_passwd(1,'123456','888','888')
