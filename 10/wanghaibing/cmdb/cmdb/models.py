...
MYSQL_user_columns='id,username,dept,sex,age,birthday,tel,email,hobby,bz,groups,status'.split(',')
...
MYSQL_pass_modify_save='update user   set password=md5(%s)  where id=%s'
...
MYSQL_monitor_create='insert into monitor_host(ip,cpu,mem,disk,m_time,r_time) values(%s,%s,%s,%s,%s,%s) '
MYSQL_MONITOR_LIST='select cpu,mem,disk,m_time from monitor_host  where ip=%s and  m_time > %s order by m_time'
...
def get_user(key=None):
   if key:
     ...
     result=[]
     users=mysql_exe(MYSQL_get_user_all)
     for date in users:
          users_dict=dict(zip(MYSQL_user_columns,date))
          users_dict['birthday']=users_dict['birthday'].strftime('%Y-%m-%d')
...
def pass_modify_save(arg):
   m = hashlib.md5()
   m.update(arg.get('pass_old2'))
   psw = m.hexdigest()
   if not arg.get('pass_old2'):
      return False,'旧密码不能为空.'
   if arg.get('pass_old1') != psw:
      return False,'旧密码不对.'
   if not arg.get('pass_new1'):
      return False,'新密码不能为空.'
   if not arg.get('pass_new2'):
      return False,'确认新密码不能为空.'
   if arg.get('pass_new1') != arg.get('pass_new2'):
      return False,'两次新密码不相符.'
   uid=arg.get('user_modify_id')
   pass_new=arg.get('pass_new1')
   mysql_exe(MYSQL_pass_modify_save,(pass_new,uid))
   return True,''  
...
def monitor_host_create(req):
     ...
     for key in ['ip','cpu','mem','disk','m_time']:
         params.append(req.get(key,''))
     params.append(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
     mysql_exe(MYSQL_monitor_create,params)
     return  True

def get_monitor_list(ip,h_limit):
      now1=datetime.datetime.now() 
      later_time=(now1-datetime.timedelta(hours=h_limit)).strftime('%Y-%m-%d %H:%M:%S')
      categ_val=[]
      cpu_val=[]
      mem_val=[]
      disk_val=[]
           
      result=mysql_exe(MYSQL_MONITOR_LIST,(ip,later_time))
      for x in result:
          categ_val.append(x[3].strftime('%H:%M:%S'))      
          cpu_val.append(x[0])      
          mem_val.append(x[1])      
          disk_val.append(x[2])      
      return {
       'categories' : categ_val,
       'series' : [{
          'name': 'CPU',
          'data': cpu_val
       }, {
